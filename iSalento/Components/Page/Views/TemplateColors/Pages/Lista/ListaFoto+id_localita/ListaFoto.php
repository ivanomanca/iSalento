...
class ListaFoto extends Isp_View_Page{
	...
	public function getRelatedIngredients(){
		if(isset($this->listaFoto[0])){
			$ingredients['oggettoLocalita'] = array("Localita", 
											array("id_localita" => 
											$this->listaFoto[0]->Fotovideo->id_localita));
			return $ingredients;
		}
	}
	...
	public function setThisPageUrl(){
		...
		// Foto description
		if (isset($this->oggettoLocalita->nome_localita)){
			$urlTitle = $this->oggettoLocalita->nome_localita;
			$urldescription = $this->txt['foto'].$this->oggettoLocalita->nome_localita;
			$urldescription .= " ( Salento ) ";
		}else{
			$urlTitle = null;
			$urldescription = null;
		}

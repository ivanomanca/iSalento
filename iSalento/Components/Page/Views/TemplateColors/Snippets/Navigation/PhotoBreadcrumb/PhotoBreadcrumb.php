 ...
 *	
 * EXAMPLE CODE:
 * 	  <div class="separatore" (id="bottom")>
          <div class="left">
...
class PhotoBreadcrumb extends Isp_View_Snippet{
	 ...
	 * @param boolean $bottom - true to activate a bottom PhotoBreadcrum
	 */
	...
	public function render(){
		// Specify if a bottom PhotoBreadcrumb is been called
		if($this->bottom){
			$idBottom = $this->idBottom;
		}else{
			$idBottom = null;
		}	
		// Div separatore
		$code = $this->openTag($idBottom , $this->classSeparator);
			// LEFT
			if(isset($this->leftArray)){
				$code .= $this->openTag(null, $this->classLeft);
					...
					if(is_int($this->leftArray[1])){
						$code .= $this->leftArray[1];
					}elseif($this->leftArray[1] instanceof Isp_Url ){
			...
			if(isset($this->avantiIndietro) or isset($this->itemsArray)){
				...
				if(isset($this->itemsArray)){
					foreach ($this->itemsArray as $link){
						if(isset($link)){
							if(is_int($link)){
								$code .= "<span> $link </span>";
